...
for files in file_mat:
	if files.endswith(".mat"):
		...
		files_name=files.split("_counts_")[0]
		for line in f :
			line=line.replace('\n','').split()
...
for tax_ids in Dic_best_hit:
	...
	for ids in Dic_best_hit[tax_ids]:
		...
		if ids in Dic_count_by_samples:
...
for tax_ids in Dic_best_hit:
	...
	for seqs in Dic_best_hit[tax_ids]:
		if seqs.startswith("k") or seqs.startswith("C") :
			...
			if number_sigleton!=0:
				percentage_id_sigleton+=float(Dic_best_hit[tax_ids][seqs][9])
			else:
				percentage_id_sigleton=0
	if num_contigs !=0 and number_sigleton!=0 :
		...
		"%.2f" % (percentage_id_sigleton/number_sigleton)))
	if num_contigs !=0 and number_sigleton==0 :
		...
		tax_hits_infos[tax_ids].extend(( "-", "-" , number_sigleton, "-", "-","-", "-", "-", "-","-","-","-", "%.2f" % (percentage_id_sigleton/number_sigleton)))
...
stat_info_brut=["qseqid", "sseqid", "qlen", "slen", "length", "qstart", "qend", "sstart", "send", "qcovhsp", "pident", "evalue", "bitscore"]
lineage_brut=[stat_info_brut+ranks+samples[1:]]
dic_lineage={}
...
for tax_id in Dic_best_hit:
	for key in Dic_best_hit[tax_id].keys():
		key=key.split('/', 1)[0]
		if key in Dic_count_by_samples:

			lineage_brut.append([key]+list_table + Dic_best_hit[tax_id][key] + dic_lineage[tax_id] + Dic_count_by_samples[key])
		else:
			continue
